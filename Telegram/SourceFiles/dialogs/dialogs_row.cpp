...
[[nodiscard]] QImage CornerBadgeTTL(not_null<PeerData *> peer,
  ...
  auto result = Images::Circle(BlurredDarkenedPart(
      ...
      QRect(QPoint(partSkip, partSkip) * ratio,
...
QRect CornerBadgeTTLRect(int photoSize) {
  ...
  return QRect(photoSize - partSize + st::dialogsTTLBadgeSkip.x(),
               photoSize - partSize + st::dialogsTTLBadgeSkip.y(), partSize,
               partSize);
}
...
void Row::updateCornerBadgeShown(not_null<PeerData *> peer,
  ...
  const auto nextLayer = [&] {
    if (hasUnreadBadgesAbove) {
      return kNoneLayer;
    } else if (user && Core::App().settings().showOnlineStatus() &&
...
void Row::PaintCornerBadgeFrame(not_null<CornerBadgeUserpic *> data,
  ...
  if (peer && Core::App().settings().showOnlineStatus()) {
  ...
  const auto skip =
      online ? st::dialogsOnlineBadgeSkip : st::dialogsCallBadgeSkip;
  ...
  q.drawEllipse(QRectF(photoSize - skip.x() - size, photoSize - skip.y() - size,
                       size, size)
...
void Row::paintUserpic(Painter &p, not_null<Entry *> entry, PeerData *peer,
  ...
  actionPainter->paintSpeaking(p, context.st->photoSize - skip.x() - size,
                               context.st->photoSize - skip.y() - size,
                               context.width, bg, context.now);
  p.translate(-context.st->padding.left(), -context.st->padding.top());
  p.setOpacity(1.);
}
...
DateText FakeRow::dateText(TimeId date, crl::time now) const {
