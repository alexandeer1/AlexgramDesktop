...
// above the given top of the visible area or below the given bottom of the
// visible area is applied once for blocks list in a history and once for items
// list in the found block.
...
// http://qt-project.org/doc/qt-4.8/demos-embedded-anomaly-src-flickcharm-cpp.html
...
public:
  ...
  elementInSelectionMode(const Element *view) override {
    if (view && view->data()->isSponsored()) {
      return HistoryView::SelectionModeResult();
    }
    return _widget ? _widget->inSelectionMode()
                   : HistoryView::SelectionModeResult();
  }
  bool elementIntersectsRange(not_null<const Element *> view, int from,
                              int till) override {
  ...
  void elementShowPollResults(not_null<PollData *> poll,
  ...
  void elementShowAddPollOption(not_null<Element *> view,
                                not_null<PollData *> poll, FullMsgId context,
  ...
  void elementOpenPhoto(not_null<PhotoData *> photo,
                        FullMsgId context) override {
    if (_widget) {
      _widget->elementOpenPhoto(photo, context);
    }
  }
  void elementOpenDocument(not_null<DocumentData *> document, FullMsgId context,
  ...
  void elementSendBotCommand(const QString &command,
                             const FullMsgId &context) override {
    if (_widget) {
      _widget->elementSendBotCommand(command, context);
    }
  }
  void elementSearchInList(const QString &query,
                           const FullMsgId &context) override {
    if (_widget) {
  ...
  void elementStartEffect(not_null<const Element *> view,
                          Element *replacing) override {
    if (_widget) {
...
HistoryInner::HistoryInner(not_null<HistoryWidget *> historyWidget,
    ...
    : RpWidget(nullptr), _widget(historyWidget), _scroll(scroll),
      _controller(controller), _peer(history->peer), _history(history),
      _elementDelegate(_history->delegateMixin()->delegate()),
      _emojiInteractions(std::make_unique<HistoryView::EmojiInteractions>(
          this, controller->content(), &controller->session(),
          [=](not_null<const Element *> view) { return itemTop(view); })),
      _migrated(history->migrateFrom()),
      _translateTracker(
          std::make_unique<HistoryView::TranslateTracker>(history)),
      _readMetricsTracker(
          std::make_unique<HistoryView::ReadMetricsTracker>(_peer)),
      _pathGradient(HistoryView::MakePathShiftGradient(controller->chatStyle(),
                                                       [=] { update(); })),
      _reactionsManager(std::make_unique<HistoryView::Reactions::Manager>(
          this, [=](QRect updated) { update(updated); })),
      _replyButtonManager(std::make_unique<HistoryView::ReplyButton::Manager>(
  ...
  Core::App().inAppKeyPressed() |
      rpl::on_next([=] { registerReadMetricsActivity(); }, lifetime());
  _controller->gifPauseLevelChanged() | rpl::on_next(
                                            [=] {
  ...
  _emojiInteractions->playStarted() |
      rpl::on_next(
          [=](QString &&emoji) {
            _controller->emojiInteractions().playStarted(_peer,
                                                         std::move(emoji));
          },
          lifetime());

  _reactionsManager->chosen() | rpl::on_next(
  ...
  session().data().itemRemoved() |
      rpl::on_next([this](auto item) { itemRemoved(item); }, lifetime());
  session().data().viewRemoved() |
      rpl::on_next([this](auto view) { viewRemoved(view); }, lifetime());
  rpl::merge(session().data().historyUnloaded(),
  ...
  session().data().viewRepaintRequest() |
      rpl::on_next(
          [this](Data::RequestViewRepaint data) {
  ...
  HistoryView::Reactions::SetupManagerList(_reactionsManager.get(),
  ...
  controller->adaptive().chatWideValue() | rpl::on_next(
                                               [=](bool wide) {
                                                 _isChatWide = wide;
                                                 if (_overlayHost) {
                                                   _overlayHost->hide();
  ...
  _selectScroll.scrolls() |
      rpl::on_next([=](int d) { _scroll->scrollToY(_scroll->scrollTop() + d); },
                   _scroll->lifetime());
...
void HistoryInner::reactionChosen(const ChosenReaction &reaction) {
  ...
  if (!item) {
    ...
    Payments::ShowPaidReactionDetails(_controller, item, viewByItem(item),
...
void HistoryInner::setupSharingDisallowed() {
  ...
  if (const auto user = _peer->asUser()) {
    _sharingDisallowed =
        ...
        rpl::map([](bool my, bool peer) { return my || peer; });
  } else {
    const auto chat = _peer->asChat();
    const auto channel = _peer->asChannel();
    _sharingDisallowed =
        chat ? Data::PeerFlagValue(chat, ChatDataFlag::NoForwards)
  ...
  if (const auto chat = _peer->asChat()) {
    ...
    auto canDelete =
        std::move(rights) | rpl::map([=] { return chat->canDeleteMessages(); });
    ...
    auto canDelete = std::move(rights) |
                     rpl::map([=] { return channel->canDeleteMessages(); });
    rpl::combine(_sharingDisallowed.value(), std::move(canDelete)) |
        rpl::on_next([=] { clearIfRestricted(); }, lifetime());
  } else {
...
void HistoryInner::setupSwipeReplyAndBack() {
  ...
  auto update = [=, history = _history](Ui::Controls::SwipeContextData data) {
    if (data.translation > 0) {
      if (!_swipeBackData.callback) {
        _swipeBackData = Ui::Controls::SetupSwipeBack(
            _widget, [=]() -> std::pair<QColor, QColor> {
    ...
    if (changed) {
      ...
      const auto item = history->peer->owner().message(history->peer->id,
  ...
  auto init = [=, show = _controller->uiShow()](int cursorTop,
    ...
    enumerateItems<EnumItemsDirection::BottomToTop>(
        [&](not_null<Element *> view, int itemtop, int itembottom) {
          if ((cursorTop < itemtop) || (cursorTop > itembottom) ||
              !view->data()->isRegular() ||
          ...
          result.callback = [=, itemId = item->fullId()] {
            ...
            const auto replyToItemId =
                (selected.item ? selected.item : still)->fullId();
...
void HistoryInner::messagesReceived(not_null<PeerData *> peer,
  ...
  if (_history->peer == peer) {
    ...
    const auto newLoaded =
        _migrated && _migrated->isEmpty() && !_history->isEmpty();
...
void HistoryInner::messagesReceivedDown(not_null<PeerData *> peer,
                                        const QVector<MTPMessage> &messages) {
  if (_history->peer == peer) {
    const auto oldLoaded =
        _migrated && _history->isEmpty() && !_migrated->isEmpty();
...
void HistoryInner::enumerateItemsInHistory(History *history, int historytop,
                                           Method method) {
  // No displayed messages in this history.
  if (historytop < 0 || history->isEmpty()) {
    return;
  }
  if (_visibleAreaBottom <= historytop ||
      historytop + history->height() <= _visibleAreaTop) {
    return;
  }

  auto searchEdge = TopToBottom ? _visibleAreaTop : _visibleAreaBottom;

  // Binary search for blockIndex of the first block that is not completely
  // below the visible area.
  auto blockIndex = BinarySearchBlocksOrItems<TopToBottom>(
      history->blocks, searchEdge - historytop);

  // Binary search for itemIndex of the first item that is not completely below
  // the visible area.
  auto block = history->blocks[blockIndex].get();
  auto blocktop = historytop + block->y();
  auto blockbottom = blocktop + block->height();
  auto itemIndex = BinarySearchBlocksOrItems<TopToBottom>(
      block->messages, searchEdge - blocktop);
  ...
  while (true) {
    while (true) {
      ...
      // Binary search should've skipped all the items that are above / below
      // the visible area.
      if (TopToBottom) {
...
bool HistoryInner::canHaveFromUserpics() const {
  if (_peer->isUser() && !_peer->isSelf() && !_peer->isRepliesChat() &&
      !_peer->isVerifyCodes() && !_isChatWide) {
    return false;
...
template <typename Method> void HistoryInner::enumerateUserpics(Method method) {
  ...
  auto userpicCallback = [&](not_null<Element *> view, int itemtop,
    ...
    if (view->displayFromPhoto() ||
        (view->hasFromPhoto() && itembottom >= _visibleAreaBottom)) {
...
template <typename Method> void HistoryInner::enumerateDates(Method method) {
  ...
  auto dateCallback = [&](not_null<Element *> view, int itemtop,
    ...
    if (view->displayDate() ||
        (!item->isEmpty() && itemtop <= _visibleAreaTop)) {
      ...
      int dateHeight = st::msgServicePadding.bottom() +
                       st::msgServiceFont->height + st::msgServicePadding.top();
...
void HistoryInner::enumerateForumThreadBars(Method method) {
  ...
  const auto skip =
      (_scrollDateOpacity.animating() || _scrollDateShown)
          ? int(base::SafeRound(
  ...
  auto barCallback = [&](not_null<Element *> view, int itemtop,
                         int itembottom) {
    const auto item = view->data();
    ...
    // Call method on a bar for all messages that have it and for those who are
    // not showing it because they are in a one day together with the previous
    // message if they are top-most visible.
    if (view->displayForumThreadBar() ||
        (!item->isEmpty() && itemtop <= _visibleAreaTop)) {
      if (lowestInOneBunchItemBottom < 0) {
        lowestInOneBunchItemBottom = itembottom - view->marginBottom();
      }
      // Attach bar to the top of the visible area with the same margin as it
      // has in service message.
      int barTop =
          qMax(itemtop + view->displayedDateHeight(), _visibleAreaTop + skip) +
...
TextSelection
HistoryInner::computeRenderSelection(not_null<const SelectedItems *> selected,
  ...
  if (const auto group = session().data().groups().find(item)) {
    ...
    for (auto i = 0; i != count; ++i) {
      ...
      if (part == item && selection != FullSelection &&
...
TextSelection HistoryInner::itemRenderSelection(not_null<Element *> view,
                                                int selfromy,
                                                int seltoy) const {
  const auto item = view->data();
...
void HistoryInner::paintEmpty(Painter &p, not_null<const Ui::ChatStyle *> st,
...
Ui::ChatPaintContext
HistoryInner::preparePaintContext(const QRect &clip) const {
  const auto visibleAreaPositionGlobal =
...
void HistoryInner::paintEvent(QPaintEvent *e) {
  ...
  _readMetricsTracker->setScreenActive(!overlapped && !pendingResized &&
  ...
  const auto historyDisplayedEmpty =
  ...
  const auto processPainted = [&](not_null<Element *> view, int top,
                                  int height) {
    ...
    const auto isUnread =
        !item->out() && item->unread(_history) && (item->history() == _history);
    const auto withReaction = item->hasUnreadReaction();
    const auto yShown = [&](int y) {
      return (_visibleAreaBottom >= y && _visibleAreaTop <= y);
    };
    const auto markShown =
        isSponsored    ? view->markSponsoredViewed(_visibleAreaBottom - top)
    ...
    _reactionsManager->recordCurrentReactionEffect(item->fullId(),
                                                   QPoint(0, top));
  };
  ...
  if (mtop >= 0) {
    auto iBlock =
        (_curHistory == _migrated ? _curBlock : (_migrated->blocks.size() - 1));
    auto block = _migrated->blocks[iBlock].get();
    auto iItem =
        (_curHistory == _migrated ? _curItem : (block->messages.size() - 1));
  ...
  enumerateUserpics([&](not_null<Element *> view, int userpicTop) {
    ...
    if (userpicTop + st::msgPhotoSize > clip.top()) {
      ...
      if (hasTranslation) {
        ...
        update(
            QRect(st::historyPhotoLeft + context.gestureHorizontal.translation,
                  userpicTop,
                  st::msgPhotoSize - context.gestureHorizontal.translation,
                  st::msgPhotoSize));
      }
      if (const auto from = item->displayFrom()) {
        Dialogs::Ui::PaintUserpic(p, from, validateVideoUserpic(from),
                                  _userpics[from], st::historyPhotoLeft,
                                  userpicTop, width(), st::msgPhotoSize,
                                  context.paused);
      } else if (const auto info = item->displayHiddenSenderInfo()) {
        if (info->customUserpic.empty()) {
          info->emptyUserpic.paintCircle(p, st::historyPhotoLeft, userpicTop,
                                         width(), st::msgPhotoSize);
        } else {
          auto &userpic = _hiddenSenderUserpics[item->id];
          const auto valid =
              info->paintCustomUserpic(p, userpic, st::historyPhotoLeft,
                                       userpicTop, width(), st::msgPhotoSize);
          if (!valid) {
      ...
      if (opacity > 0.) {
        ...
        if (const auto date = view->Get<HistoryView::DateBadge>()) {
          ...
          HistoryView::ServiceMessagePainter::PaintDate(
              p, context.st, view->dateTime(), dateY, _contentWidth,
              _isChatWide);
        }
      }
    }
  ...
  enumerateForumThreadBars([&](not_null<Element *> view, int itemtop,
                               int barTop) {
    ...
    if (barInPlace) {
      const auto correctBarTop =
          itemtop + view->displayedDateHeight() + st::msgServiceMargin.top();
    ...
    if (barTop < clip.top() + clip.height()) {
      ...
      if (const auto bar = view->Get<HistoryView::ForumThreadBar>()) {
        bar->paint(p, context.st, barY, _contentWidth, _isChatWide,
                   !barInPlace);
      } else {
        _forumThreadBarWidth = HistoryView::ForumThreadBar::PaintForGetWidth(
            p, context.st, view, _forumThreadBarUserpicView, barY,
            _contentWidth, _isChatWide);
      }
    }
    return true;
  });
...
int HistoryInner::SelectionViewOffset(not_null<const HistoryInner *> inner,
                                      not_null<const Element *> view) {
...
HistoryInner::VideoUserpic *
HistoryInner::validateVideoUserpic(not_null<PeerData *> peer) {
    ...
    enumerateUserpics([&](not_null<Element *> view, int userpicTop) {
      ...
      if (userpicTop + st::msgPhotoSize > _visibleAreaTop) {
        if (const auto from = view->data()->displayFrom()) {
          if (from == peer) {
            rtlupdate(st::historyPhotoLeft, userpicTop, st::msgPhotoSize,
                      st::msgPhotoSize);
          }
        }
      }
  ...
  return _videoUserpics
      .emplace(peer, std::make_unique<VideoUserpic>(peer, repaint))
      .first->second.get();
}
...
void HistoryInner::touchUpdateSpeed() {
  ...
  if (_touchPrevPosValid) {
    ...
    if (elapsed) {
      ...
      // fingers are inacurates, we ignore small changes to avoid stopping the
      // autoscroll because of a small horizontal offset when scrolling
      // vertically
      const int newSpeedY =
...
void HistoryInner::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (reallyMoved) {
    ...
    lastGlobalPosition = e->globalPos();
    if (!buttonsPressed ||
        (_scrollDateLink && ClickHandler::getPressed() == _scrollDateLink)) {
...
void HistoryInner::mouseActionStart(const QPoint &screenPos,
  ...
  _dragStartPosition =
      mapPointToItem(mapFromGlobal(screenPos), mouseActionView);
      ...
      if (Element::Pressed()) {
        ...
        if (uponSelected) {
          if (_selected.empty() ||
            ...
            uint16 selFrom = _selected.cbegin()->second.from,
                   selTo = _selected.cbegin()->second.to;
        ...
        if (uponSelected) {
          ...
          if (_mouseCursorState == CursorState::Date) {
            _mouseAction =
                MouseAction::PrepareDrag; // start sticker drag or by-date drag
          } else {
...
std::unique_ptr<QMimeData> HistoryInner::prepareDrag() {
  ...
  if (auto mimeData = TextUtilities::MimeDataFromText(selectedText)) {
    ...
    const auto tryForwardSelection =
        uponSelected && !_controller->adaptive().isOneColumn();
    const auto forwardSelectionState =
        tryForwardSelection ? getSelectionState()
    ...
    if (pressedHandler) {
      const auto lnkDocument = reinterpret_cast<DocumentData *>(
          pressedHandler->property(kDocumentLinkMediaProperty).toULongLong());
      if (lnkDocument) {
...
void HistoryInner::mouseActionFinish(const QPoint &screenPos,
                                     Qt::MouseButton button) {
  mouseActionUpdate(screenPos);
  ...
  if (_mouseAction == MouseAction::Dragging) {
    ...
    if (_mouseAction == MouseAction::PrepareDrag && !_pressWasInactive &&
        inSelectionMode().inSelectionMode && button != Qt::RightButton) {
  ...
  if ((_mouseAction == MouseAction::PrepareSelect) && !_pressWasInactive &&
      inSelectionMode().inSelectionMode) {
    changeSelectionAsGroup(&_selected, _mouseActionItem, SelectAction::Invert);
    repaintItem(_mouseActionItem);
  } else if ((_mouseAction == MouseAction::PrepareDrag) && !_pressWasInactive &&
             _dragStateItem && (button != Qt::RightButton)) {
    auto i = _selected.find(_dragStateItem);
    if (i != _selected.cend() && i->second == FullSelection) {
      _selected.erase(i);
      repaintItem(_mouseActionItem);
    } else if ((i == _selected.cend()) && !_dragStateItem->isService() &&
      ...
      if (_selected.size() < MaxSelectedItems) {
        ...
        repaintItem(_mouseActionItem);
               ...
               !hasSelectRestriction() && _dragStateItem->isRegular() &&
  ...
  if (QGuiApplication::clipboard()->supportsSelection() && !_selected.empty() &&
    ...
    if (const auto view = viewByItem(item)) {
      TextUtilities::SetClipboardText(view->selectedText(selection),
...
void HistoryInner::mouseDoubleClickEvent(QMouseEvent *e) {
  ...
  if (!ClickHandler::getActive() && !ClickHandler::getPressed() &&
      ...
      !inSelectionMode().inSelectionMode && !_emptyPainter &&
...
void HistoryInner::toggleFavoriteReaction(not_null<Element *> view) const {
  ...
  if (!ranges::contains(Data::LookupPossibleReactions(item).recent, favorite,
                        &Data::Reaction::id) ||
...
HistoryInner::selectedQuote(not_null<HistoryItem *> item) const {
  if (_selected.size() != 1 || _selected.begin()->first != item ||
...
void HistoryInner::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
  ...
  if (_controller->showFrozenError()) {
    return;
             ...
             !link->property(kSendReactionEmojiProperty)
                  .value<Data::ReactionId>()
                  .empty() &&
             _reactionsManager->showContextMenu(
                 this, e, session().data().reactions().favoriteId())) {
    return;
    ...
    if (const auto item = _dragStateItem) {
      ...
      const auto reactionsSkip =
          view->embedReactionsInBubble()
              ? 0
              : view->reactionButtonParameters({}, {}).reactionsHeight;
      const auto top = itemTop(view) + view->height() - reactionsSkip -
                       _visibleAreaTop - rightSize.height();
      const auto right = rect::right(view->innerGeometry()) -
                         st::historyFastShareLeft - rightSize.width();
      HistoryView::ShowTopPeersSelector(
          this, _controller->uiShow(), item->fullId(),
  ...
  // -2 - has full selected items, but not over, -1 - has selection, but no
  // over, 0 - no selection, 1 - over text, 2 - over full selected items
  ...
  const auto hasWhoReactedItem =
      leaderOrSelf &&
      Api::WhoReactedExists(leaderOrSelf, Api::WhoReactedList::All);
  using namespace HistoryView::Reactions;
  const auto clickedReaction = ReactionIdOfLink(link);
  const auto linkPhoneNumber =
      link ? link->property(kPhoneNumberLinkProperty).toString() : QString();
  const auto linkUserpicPeerId =
      (link && _dragStateUserpic)
          ? link->property(kPeerLinkPeerIdProperty).toULongLong()
          : 0;
  const auto todoListTaskId =
      link ? link->property(kTodoListItemIdProperty).toInt() : 0;
  const auto pollOptionLink =
      link ? link->property(kPollOptionProperty).toByteArray() : QByteArray();
  const auto session = &this->session();
  _whoReactedMenuLifetime.destroy();
  if (!clickedReaction.empty() && leaderOrSelf) {
    if (clickedReaction.paid()) {
      Payments::ShowPaidReactionDetails(_controller, leaderOrSelf,
                                        viewByItem(leaderOrSelf),
                                        HistoryReactionSource::Selector);
      e->accept();
      return;
    } else if (Api::WhoReactedExists(leaderOrSelf, Api::WhoReactedList::One)) {
      HistoryView::ShowWhoReactedMenu(&_menu, e->globalPos(), this,
                                      leaderOrSelf, clickedReaction,
                                      _controller, _whoReactedMenuLifetime);
      e->accept();
      return;
    } else if (HistoryView::ShowReactionPreview(
                   _controller, leaderOrSelf->fullId(), clickedReaction)) {
      return;
    }
  }
  if (!linkPhoneNumber.isEmpty()) {
    PhoneClickHandler(session, linkPhoneNumber)
        .onClick(prepareClickContext(Qt::LeftButton,
                                     _dragStateItem ? _dragStateItem->fullId()
                                                    : FullMsgId()));
    return;
  }
  ...
  if (linkUserpicPeerId) {
    _widget->fillSenderUserpicMenu(
        _menu.get(), session->data().peer(PeerId(linkUserpicPeerId)));
  ...
  const auto addItemActions = [&](HistoryItem *item,
    ...
    if (!item || !item->isRegular() || isUponSelected == 2 ||
        isUponSelected == -2) {
      return;
    }
    ...
    const auto topicRootId =
        item->history()->isForum() ? item->topicRootId() : 0;
    if (topicRootId || (withReplies && item->history()->peer->isMegagroup())) {
      const auto highlightId = topicRootId ? item->id : 0;
      const auto rootId = topicRootId    ? topicRootId
                          : repliesCount ? item->id
                                         : item->replyToTop();
      const auto phrase =
          topicRootId ? tr::lng_replies_view_topic(tr::now)
          : (repliesCount > 0)
              ? tr::lng_replies_view(tr::now, lt_count, repliesCount)
    ...
    const auto pinItem =
        (item->canPin() && item->isPinned()) ? item : groupLeaderOrSelf(item);
    if (pinItem->canPin()) {
      const auto isPinned = pinItem->isPinned();
      const auto pinItemId = pinItem->fullId();
      _menu->addAction(isPinned ? tr::lng_context_unpin_msg(tr::now)
                                : tr::lng_context_pin_msg(tr::now),
                       crl::guard(controller,
                                  [=] {
                                    Window::ToggleMessagePinned(
                                        controller, pinItemId, !isPinned);
                                  }),
                       isPinned ? &st::menuIconUnpin : &st::menuIconPin);
    }
    if (!item->isService() && peerIsChannel(itemId.peer) &&
        !_peer->isMegagroup()) {
      ...
      if (const auto channel = _peer->asChannel()) {
        if ((channel->flags() & ChannelDataFlag::CanGetStatistics) ||
          ...
          _menu->addAction(tr::lng_stats_title(tr::now), std::move(callback),
                           &st::menuIconStats);
        }
      }
    }
    ...
    if (!photo->isNull() && media && media->loaded() &&
      ...
      _menu->addAction(
          ...
          base::fn_delayed(st::defaultDropdownMenu.menu.ripple.hideDuration,
                           this, [=] { savePhotoToFile(photo); }),
          &st::menuIconSaveImage);
      _menu->addAction(
          tr::lng_context_copy_image(tr::now),
          [=] { copyContextImage(photo, itemId); }, &st::menuIconCopy);
    }
  ...
  const auto addDocumentActions = [&](not_null<DocumentData *> document,
    ...
    if (document->loading()) {
      _menu->addAction(
          ...
          [=] { cancelContextDownload(document); }, &st::menuIconCancel);
      return;
    }
    const auto itemId = item ? item->fullId() : FullMsgId();
    if (document->isGifv()) {
      const auto notAutoplayedGif = [&] {
        return item && !Data::AutoDownload::ShouldAutoPlay(
                           session->settings().autoDownload(),
                           item->history()->peer, document);
      }();
      if (notAutoplayedGif) {
        _menu->addAction(
            tr::lng_context_open_gif(tr::now), [=] { openContextGif(itemId); },
            &st::menuIconShowInChat);
      }
      if (!hasCopyMediaRestriction(item)) {
        _menu->addAction(
            tr::lng_context_save_gif(tr::now), [=] { saveContextGif(itemId); },
            &st::menuIconGif);
      }
    }
    if (!document->filepath(true).isEmpty()) {
      _menu->addAction(
          ...
          [=] { showContextInFolder(document); }, &st::menuIconShowInFolder);
    }
    if (item && !hasCopyMediaRestriction(item) &&
        !HistoryView::ItemHasTtl(item)) {
      HistoryView::AddSaveSoundForNotifications(_menu, item, document,
                                                controller);
      HistoryView::AddSaveDocumentAction(
          Ui::Menu::CreateAddActionCallback(_menu), item, document, controller);
      HistoryView::AddCopyFilename(
          _menu, document, [=] { return showCopyRestrictionForSelected(); });
    }
  ...
  const auto asGroup =
      !Element::Moused() || (Element::Moused() != Element::Hovered()) ||
  ...
  const auto addSelectMessageAction = [&](not_null<HistoryItem *> item) {
    if (item->isRegular() && !item->isService() && !hasSelectRestriction()) {
      ...
      _menu->addAction(
          ...
          [=] {
            if (const auto item = session->data().message(itemId)) {
              if ([[maybe_unused]] const auto view = viewByItem(item)) {
                if (asGroup) {
                  changeSelectionAsGroup(&_selected, item,
                                         SelectAction::Select);
                } else {
          ...
          &st::menuIconSelect);
      const auto collectBetween = [=](not_null<HistoryItem *> from,
      ...
      [&] { // Select up to this message.
        ...
        auto nearestItem = (HistoryItem *)(nullptr);
        {
          ...
          for (const auto &[item, _] : _selected) {
            const auto diff =
                item->fullId().msg.bare - toItem->fullId().msg.bare;
        ...
        const auto left =
            MaxSelectedItems - selectedState.count + (topToBottom ? 0 : 1);
        ...
        _menu->addAction(tr::lng_context_select_msg_bulk(tr::now), callback,
                         &st::menuIconSelect);
      }();
    }
  };
  ...
  const auto addReplyAction = [&](HistoryItem *item) {
    ...
    if (canReply) {
      ...
      auto text = (selected         ? tr::lng_context_quote_and_reply
          ...
          tr::now, Ui::Text::FixAmpersandInAction);
      ...
      if (canSendReply && document && document->isVoiceMessage()) {
        ...
        if (const auto t = HistoryView::CurrentVoiceTimecode(msgId)) {
          Menu::AddTimecodeAction(
              _menu.get(), *t, HistoryView::VoiceTimecodeUpdates(msgId), [=] {
  ...
  if (lnkPhoto || lnkDocument) {
    ...
    if (isUponSelected > 0) {
      ...
      if (!hasCopyRestrictionForSelected() && !selectedText.empty()) {
        _menu->addAction((isUponSelected > 1
                              ? tr::lng_context_copy_selected_items(tr::now)
                              : tr::lng_context_copy_selected(tr::now)),
                         [=] { copySelectedText(); }, &st::menuIconCopy);
      }
    ...
    if (item && item->hasDirectLink() && isUponSelected != 2 &&
        isUponSelected != -2) {
      _menu->addAction(
          ...
          [=] {
            HistoryView::CopyPostLink(controller, itemId,
    ...
    if (isUponSelected > 1) {
      if (selectedState.count > 0 &&
        ...
        _menu->addAction(
            ...
            [=] { _widget->forwardSelected(); }, &st::menuIconForward);
      }
      if (selectedState.count > 0 &&
          selectedState.canDeleteCount == selectedState.count) {
        _menu->addAction(
            tr::lng_context_delete_selected(tr::now),
            [=] { _widget->confirmDeleteSelected(); }, &st::menuIconDelete);
      }
      ...
      _menu->addAction(
          ...
          [=] { _widget->clearSelected(); }, &st::menuIconSelect);
    } else if (item) {
      const auto itemId = item->fullId();
      ...
      if (isUponSelected != -2) {
        if (item->allowsForward()) {
          _menu->addAction(
              ...
              [=] { forwardItem(itemId); }, &st::menuIconForward);
        }
      ...
      if (isUponSelected != -2 && blockSender) {
        _menu->addAction(
            ...
            [=] { blockSenderItem(itemId); }, &st::menuIconBlock);
      }
    }
    ...
    const auto partItemOrLeader =
        (asGroup || !albumPartItem) ? item : albumPartItem;
    const auto itemId = item ? item->fullId() : FullMsgId();
    const auto canDelete =
        item && item->canDelete() && (item->isRegular() || !item->isService());
    ...
    const auto sponsored =
        (item && item->isSponsored()) ? item
        : (Element::Moused() && Element::Moused()->data()->isSponsored())
            ? Element::Moused()->data().get()
            : nullptr;
    if (sponsored) {
      Menu::FillSponsored(Ui::Menu::CreateAddActionCallback(_menu),
                          controller->uiShow(), sponsored->fullId());
    }
    if (isUponSelected > 0) {
      addReplyAction(item);
      const auto selectedText = getSelectedText();
      ...
      if (item && !isUponSelected) {
          ...
          if (!view->isIsolatedEmoji() && document->sticker()) {
            if (document->sticker()->set) {
              ...
              Api::AddAddToOwnedSetAction(
                  ...
                  _controller->uiShow(), document);
            }
            {
              const auto isFaved = session->data().stickers().isFaved(document);
              _menu->addAction(
                  isFaved ? tr::lng_faved_stickers_remove(tr::now)
                          : tr::lng_faved_stickers_add(tr::now),
                  [=] {
                    Api::ToggleFavedSticker(controller->uiShow(), document,
                                            itemId);
                  },
        ...
        if (const auto media = item->media()) {
          if (const auto poll = media->poll()) {
            HistoryView::AddPollActions(_menu, poll, item,
                                        HistoryView::Context::History,
                                        _controller, !pollOptionLink.isEmpty());
            ...
            if (!user ||
              ...
              const auto controller = _controller;
              const auto starGiftUpgrade =
                  gift->upgrade && (gift->type == Data::GiftType::StarGift);
              const auto isGift = gift->slug.isEmpty() || !gift->channel;
              const auto out = item->out();
              const auto outgoingGift =
                  isGift && (starGiftUpgrade ? !out : out);
              if (outgoingGift &&
    ...
    if (!actionText.isEmpty()) {
      _menu->addAction(
          ...
          &st::menuIconCopy);
    } else if (item && item->hasDirectLink() && isUponSelected != 2 &&
               isUponSelected != -2) {
      _menu->addAction(
          item->history()->peer->isMegagroup()
              ? tr::lng_context_copy_message_link(tr::now)
    ...
    if (sponsored) {
      const auto hasAbout = ranges::any_of(
          ...
          [about = tr::lng_sponsored_menu_revenued_about(tr::now)](
              const QAction *action) { return action->text() == about; });
      if (!hasAbout) {
        if (!_menu->empty()) {
          _menu->addSeparator(&st::expandedMenuSeparator);
        }
        auto item = base::make_unique_q<Ui::Menu::MultilineAction>(
            _menu->menu(), st::menuWithIcons, st::historyHasCustomEmoji,
            st::historySponsoredAboutMenuLabelPosition,
            TextWithEntities{tr::lng_sponsored_title(tr::now)},
            &st::menuIconInfo);
        item->clicks() |
            rpl::on_next([=] { controller->show(Box(Ui::AboutSponsoredBox)); },
                         item->lifetime());
    ...
    if (isUponSelected > 1) {
      ...
      _menu->addAction(
          tr::lng_context_clear_selection(tr::now),
          [=] { _widget->clearSelected(); }, &st::menuIconSelect);
    } else if (item && ((isUponSelected != -2 && (canForward || canDelete)) ||
                        item->isRegular())) {
      if (isUponSelected != -2) {
        if (canForward) {
          _menu->addAction(
              tr::lng_context_forward_msg(tr::now),
              [=] { forwardAsGroup(itemId); }, &st::menuIconForward);
        }
        if (HistoryView::CanAddOfferToMessage(item)) {
          _menu->addAction(
              tr::lng_context_add_offer(tr::now),
        ...
        if (!canBlockSender && canReport) {
          _menu->addAction(
              ...
              [=] { reportAsGroup(itemId); }, &st::menuIconReport);
        }
      }
      ...
      if (isUponSelected != -2 && canBlockSender) {
        _menu->addAction(
            tr::lng_profile_block_user(tr::now),
            [=] { blockSenderAsGroup(itemId); }, &st::menuIconBlock);
      }
  ...
  if (_dragStateItem) {
    ...
    HistoryView::AddEmojiPacksAction(
        _menu, textItem ? textItem : _dragStateItem,
        HistoryView::EmojiPacksSource::Message, _controller);
    const auto added = (_menu->actions().size() > wasAmount);
    HistoryView::AddSelectRestrictionAction(
        _menu, textItem ? textItem : _dragStateItem, !added);
  }
  ...
  if (!_menu->empty() && rateTranscriptionItem) {
    _menu->insertAction(
        0, base::make_unique_q<Menu::RateTranscribe>(
               _menu, _menu->st().menu,
  ...
  if (!pollOptionLink.isEmpty() && leaderOrSelf && !_menu->empty()) {
    ...
    _menu->stashContent([=](not_null<Ui::PopupMenu *> menu) {
      HistoryView::FillPollOptionPage(menu, &session->data(), pollItemId,
                                      pollOptionLink, _controller, [=] {
                                        _widget->replyToMessage({
  ...
  const auto reactItem =
      Element::Hovered() ? Element::Hovered()->data().get() : nullptr;
  const auto attached =
      reactItem
          ? AttachSelectorToMenu(
                _menu.get(), controller, desiredPosition, reactItem,
...
bool HistoryInner::showCopyRestriction(HistoryItem *item) {
  ...
  _controller->showToast(
      _peer->isBroadcast() ? tr::lng_error_nocopy_channel(tr::now)
...
void HistoryInner::copyContextImage(not_null<PhotoData *> photo,
  ...
  const auto restricted =
      item ? showCopyMediaRestriction(item) : IsServerMsgId(itemId.msg);
  if (photo->isNull() || !media || !media->loaded()) {
    return;
  } else if (!restricted) {
...
void HistoryInner::saveDocumentToFile(FullMsgId contextId,
                                      not_null<DocumentData *> document) {
  DocumentSaveClickHandler::SaveAndTrack(
      contextId, document, DocumentSaveClickHandler::Mode::ToNewFile);
}
...
void HistoryInner::saveContextGif(FullMsgId itemId) {
  if (const auto item = session().data().message(itemId)) {
    if (!hasCopyMediaRestriction(item)) {
      if (const auto media = item->media()) {
        if (const auto document = media->document()) {
          Api::ToggleSavedGif(_controller->uiShow(), document, item->fullId(),
                              true);
        }
      }
    }
            ...
            .emplace(item->position(),
                     Part{
                         ...
                         .time = QString("[%1] ").arg(QLocale().toString(
                             ItemDateTime(item), QLocale::ShortFormat)),
                         .unwrapped = std::move(unwrapped),
                     })
            .first;
    fullSize += i->second.time.size() + i->second.name.size() + 2 +
...
void HistoryInner::recountHistoryGeometry(bool initial) {
  ...
  auto oldHistoryMarginTop =
      qMax(visibleHeight - historyHeight() - _historyMarginBottom, 0);
  ...
  if (const auto view = _aboutView ? _aboutView->view() : nullptr) {
    ...
    if (aboutAboveHistory) {
      _aboutView->top =
          qMin(_historyMarginTop - _aboutView->height,
               qMax(0, (_scroll->height() - _aboutView->height) / 2));
    } else {
      _aboutView->top =
          qMax(qMax(0, (_scroll->height() - _aboutView->height) / 2),
...
bool HistoryInner::displayScrollDate() const {
  return (_visibleAreaTop <=
          height() - 2 * (_visibleAreaBottom - _visibleAreaTop));
}
...
void HistoryInner::scrollDateCheck() {
  ...
  auto newScrollDateItemTop =
      _history->scrollTopItem ? _history->scrollTopOffset
  ...
  //_history->blocks.back()->messages.back()->data()->date.date()) {
  ...
  if (!newScrollDateItem) {
    ...
    scrollDateHide();
  } else if (newScrollDateItem != _scrollDateLastItem ||
             newScrollDateItemTop != _scrollDateLastItemTop) {
...
void HistoryInner::keepScrollDateForNow() {
  if (!_scrollDateShown && _scrollDateLastItem &&
...
void HistoryInner::repaintScrollDateCallback() {
  ...
  int updateHeight = st::msgServiceMargin.top() + st::msgServicePadding.top() +
                     st::msgServiceFont->height +
...
void HistoryInner::updateSize() {
  ...
  auto newHistoryMarginTop =
      qMax(visibleHeight - itemsHeight - newHistoryMarginBottom, 0);
  if (aboutAboveHistory) {
    accumulate_max(newHistoryMarginTop, _aboutView->height);
  }
  ...
  if (_aboutView && _aboutView->height > 0) {
    if (aboutAboveHistory) {
      _aboutView->top =
          qMin(newHistoryMarginTop - _aboutView->height,
               qMax(0, (_scroll->height() - _aboutView->height) / 2));
    } else {
      _aboutView->top =
          qMax(qMax(0, (_scroll->height() - _aboutView->height) / 2),
               (newHistoryMarginTop + itemsHeight + newHistoryMarginBottom -
                _aboutView->height));
    }
  }
...
void HistoryInner::adjustCurrent(int32 y, History *history) const {
  ...
  while (history->blocks[_curBlock]->y() +
                 history->blocks[_curBlock]->height() <=
             y &&
  ...
  while (block->messages[_curItem]->y() + block->messages[_curItem]->height() +
                 by <=
             y &&
...
auto HistoryInner::prevItem(Element *view) -> Element * {
  if (!view) {
    ...
    return result;
  } else if (view->history() == _history && _migrated &&
             _history->loadedAtTop() && !_migrated->isEmpty() &&
...
auto HistoryInner::nextItem(Element *view) -> Element * {
  if (!view) {
    ...
    return result;
  } else if (view->history() == _migrated && _migrated->loadedAtBottom() &&
             _history->loadedAtTop() && !_history->isEmpty()) {
...
HistoryView::SelectionModeResult HistoryInner::inSelectionMode() const {
  ...
  if (_lastInSelectionMode != now) {
    ...
    if (_inSelectionModeAnimation.animating()) {
      const auto progress = !now ? _inSelectionModeAnimation.value(0.)
                                 : 1. - _inSelectionModeAnimation.value(0.);
      _inSelectionModeAnimation.change(now ? 1. : 0.,
      ...
      _inSelectionModeAnimation.start(
          [this] {
            const_cast<HistoryInner *>(this)->update(
                QRect(0, _visibleAreaTop, width(),
                      _visibleAreaBottom - _visibleAreaTop));
          },
          now ? 0. : 1., now ? 1. : 0., st::universalDuration);
    }
  }
  return {now, _inSelectionModeAnimation.value(now ? 1. : 0.)};
}

bool HistoryInner::elementIntersectsRange(not_null<const Element *> view,
                                          int from, int till) const {
  const auto top = itemTop(view);
  if (top < 0) {
    return false;
  }
...
void HistoryInner::elementShowPollResults(not_null<PollData *> poll,
                                          FullMsgId context) {
...
void HistoryInner::elementShowAddPollOption(
  ...
  HistoryView::ShowAddPollOptionOverlay(ensureOverlayHost(), this, view, poll,
                                        context, _controller,
...
void HistoryInner::elementOpenPhoto(not_null<PhotoData *> photo,
                                    FullMsgId context) {
  const auto draw =
      Data::CanSendAnyOf(_history->peer, Data::FilesSendRestrictions());
  _controller->openPhoto(photo, {.id = context, .showDrawButton = draw});
}

void HistoryInner::elementOpenDocument(not_null<DocumentData *> document,
                                       FullMsgId context,
                                       bool showInMediaView) {
  const auto showDrawButton =
      Data::CanSendAnyOf(_history->peer, Data::FilesSendRestrictions());
  _controller->openDocument(document, showInMediaView,
...
void HistoryInner::elementSendBotCommand(const QString &command,
                                         const FullMsgId &context) {
  _widget->sendBotCommand({_history->peer, command, context});
}

void HistoryInner::elementSearchInList(const QString &query,
                                       const FullMsgId &context) {
...
HistoryView::ElementChatMode HistoryInner::elementChatMode() {
  using Mode = HistoryView::ElementChatMode;
  return _isChatWide           ? Mode::Wide
         : _removeFromUserpics ? Mode::Narrow
                               : Mode::Default;
}
...
void HistoryInner::elementStartEffect(not_null<const Element *> view,
                                      Element *replacing) {
...
void HistoryInner::clearSelected(bool onlyTextSelection) {
  if (!_selected.empty() &&
      (!onlyTextSelection || _selected.cbegin()->second != FullSelection)) {
    _selected.clear();
    _widget->updateTopBarSelection();
    _widget->update();
  }
...
auto HistoryInner::reactionButtonParameters(
    not_null<const Element *> view, QPoint position,
  ...
  const auto top = itemTop(view);
  if (top < 0 || !view->data()->canReact() ||
      _mouseAction == MouseAction::Dragging ||
      _mouseAction == MouseAction::Selecting ||
      inSelectionMode().inSelectionMode) {
    return {};
  }
  auto result = view->reactionButtonParameters(position, reactionState)
                    .translated({0, top});
...
auto HistoryInner::replyButtonParameters(
  ...
  const auto top = itemTop(view);
  if (top < 0 || _mouseAction == MouseAction::Dragging ||
      inSelectionMode().inSelectionMode) {
    return {};
  }
  auto result =
      view->replyButtonParameters(position, replyState).translated({0, top});
  result.visibleTop = _visibleAreaTop;
  result.visibleBottom = _visibleAreaBottom;
  result.globalPointer = _mousePosition;
  return result;
...
void HistoryInner::mouseActionUpdate() {
  ...
  const auto selectionViewOffset =
      view ? QPoint(SelectionViewOffset(this, view), 0) : QPoint(0, 0);
  ...
  if (view) {
    ...
    _reactionsManager->updateButton(
        reactionButtonParameters(view, m, reactionState));
    _replyButtonManager->updateButton(
        replyButtonParameters(view, m, replyBtnState));
    if (changed) {
      _reactionsItem = item;
    }
  ...
  auto selectingText = (item == _mouseActionItem) &&
                       (view == Element::Hovered()) && !_selected.empty() &&
                       (_selected.cbegin()->second != FullSelection);
  ...
  if (overReaction) {
    ...
    if (item != _mouseActionItem ||
        ((m + selectionViewOffset) - _dragStartPosition).manhattanLength() >=
            QApplication::startDragDistance()) {
      ...
      if (dateTop <= point.y()) {
        ...
        if (opacity > 0.) {
          ...
          if (const auto date = view->Get<HistoryView::DateBadge>()) {
            ...
            dateWidth = st::msgServiceFont->width(
                langDayOfMonthFull(view->dateTime().date()));
          }
          dateWidth +=
          ...
          if (_isChatWide) {
            maxwidth =
                qMin(maxwidth, int32(st::msgMaxWidth + 2 * st::msgPhotoSkip +
                                     2 * st::msgMargin.left()));
          }
          auto widthForDate = maxwidth - st::msgServiceMargin.left() -
          ...
          if (point.x() >= dateLeft && point.x() < dateLeft + dateWidth) {
            if (!_scrollDateLink) {
              _scrollDateLink = std::make_shared<Window::DateClickHandler>(
                  item->history(), view->dateTime().date());
            } else {
    ...
    if (!dragState.link) {
      enumerateForumThreadBars([&](not_null<Element *> view, int itemtop,
        ...
        auto barInPlace = displayBar;
        if (barInPlace) {
          const auto correctBarTop = itemtop + view->displayedDateHeight() +
                                     st::msgServiceMargin.top();
          barInPlace = (barTop < correctBarTop + st::msgServiceMargin.top());
        }
        ...
        if (barTop <= point.y()) {
          ...
          auto widthForBar = maxwidth - st::msgServiceMargin.left() -
                             st::msgServiceMargin.left();
          ...
          if (point.x() >= barLeft && point.x() < barLeft + barWidth) {
            if (!_forumThreadBarLink) {
              _forumThreadBarLink =
                  std::make_shared<Window::ForumThreadClickHandler>(item);
            } else {
              static_cast<Window::ForumThreadClickHandler *>(
                  _forumThreadBarLink.get())
                  ->update(item);
            }
    ...
    if (!dragState.link) {
      ...
      _dragStateItem = session().data().message(dragState.itemId);
      lnkhost = view;
      if (!dragState.link && m.x() >= st::historyPhotoLeft &&
          m.x() < st::historyPhotoLeft + st::msgPhotoSize) {
  ...
  if (_mouseAction == MouseAction::None) {
    ...
    if (_mouseAction == MouseAction::Selecting) {
      if (selectingText) {
        ...
        auto selState = TextSelection{qMin(second, _mouseTextSymbol),
        ...
        if (!_wasSelectedText &&
            (selState == FullSelection || selState.from != selState.to)) {
        ...
        auto selectingDown =
            (itemTop(_mouseActionItem) < itemTop(item)) ||
            (_mouseActionItem == item && _dragStartPosition.y() < m.y());
        auto dragSelFrom = viewByItem(_mouseActionItem);
        auto dragSelTo = view;
        // Maybe exclude dragSelFrom.
        if (dragSelFrom->pointState(_dragStartPosition) ==
            PointState::Outside) {
          if (selectingDown) {
            if (_dragStartPosition.y() >=
                    dragSelFrom->height() - dragSelFrom->marginBottom() ||
                ((view == dragSelFrom) &&
                 (m.y() < _dragStartPosition.y() +
                              QApplication::startDragDistance() ||
                  m.y() < dragSelFrom->marginTop()))) {
              dragSelFrom =
                  (dragSelFrom != dragSelTo) ? nextItem(dragSelFrom) : nullptr;
            }
          } else {
                ...
                ((view == dragSelFrom) &&
                 (m.y() >= _dragStartPosition.y() -
                  ...
                  m.y() >=
                      dragSelFrom->height() - dragSelFrom->marginBottom()))) {
              dragSelFrom =
                  (dragSelFrom != dragSelTo) ? prevItem(dragSelFrom) : nullptr;
            }
          }
        }
        if (_mouseActionItem != item) { // maybe exclude dragSelTo
          if (selectingDown) {
            if (m.y() < dragSelTo->marginTop()) {
              dragSelTo =
                  (dragSelFrom != dragSelTo) ? prevItem(dragSelTo) : nullptr;
            }
          } else {
            if (m.y() >= dragSelTo->height() - dragSelTo->marginBottom()) {
              dragSelTo =
                  (dragSelFrom != dragSelTo) ? nextItem(dragSelTo) : nullptr;
            }
          }
        }
        auto dragSelecting = false;
        auto dragFirstAffected = dragSelFrom;
        while (dragFirstAffected && (!dragFirstAffected->data()->isRegular() ||
                                     dragFirstAffected->data()->isService())) {
          dragFirstAffected =
              (dragFirstAffected != dragSelTo)
                  ? (selectingDown ? nextItem(dragFirstAffected)
  ...
  if (_mouseAction == MouseAction::Selecting) {
    _selectScroll.checkDeltaScroll(mousePos, _scroll->scrollTop(),
...
void HistoryInner::updateDragSelection(Element *dragSelFrom, Element *dragSelTo,
                                       bool dragSelecting) {
  if (_dragSelFrom == dragSelFrom && _dragSelTo == dragSelTo &&
...
int HistoryInner::historyHeight() const {
  ...
  if (_history->isEmpty()) {
    ...
    result += _history->height() - _historySkipHeight +
              (_migrated ? _migrated->height() : 0);
  }
  return result;
}
...
int HistoryInner::historyScrollTop() const {
  ...
  if (mtop >= 0 && _migrated->scrollTopItem) {
    return mtop + _migrated->scrollTopItem->block()->y() +
           _migrated->scrollTopItem->y() + _migrated->scrollTopOffset;
  }
  return ScrollMax;
}
...
int HistoryInner::historyTop() const {
  ...
  return !_history->isEmpty()
             ? (mig >= 0 ? (mig + _migrated->height() - _historySkipHeight)
                         : _historyMarginTop)
             : -1;
}
...
auto HistoryInner::viewByItem(const HistoryItem *item) const -> Element * {
  return !item                                        ? nullptr
         : (_aboutView && _aboutView->item() == item) ? _aboutView->view()
                                                      : item->mainView();
}
...
int HistoryInner::itemTop(const Element *view) const {
  ...
  const auto top = (view->history() == _history)
                       ? historyTop()
                       : (view->history() == _migrated ? migratedTop() : -2);
...
auto HistoryInner::findViewForPinnedTracking(int top) const
  ...
  const auto fromHistory = [&](not_null<History *> history,
                               int historyTop) -> std::pair<Element *, int> {
  ...
  const auto refresh = [&] {
    ...
    if (!_aboutView) {
      _aboutView = std::make_unique<HistoryView::AboutView>(
          _history, _history->delegateMixin()->delegate());
      _aboutView->refreshRequests() |
          rpl::on_next([=] { updateBotInfo(); }, _aboutView->lifetime());
      _aboutView->destroyRequests() | rpl::on_next(
                                          ...
                                          _aboutView->lifetime());
      _aboutView->sendIntroSticker() |
          rpl::start_to_stream(_sendIntroSticker, _aboutView->lifetime());
    }
  };
  if (const auto user = _peer->asUser()) {
    if (const auto info = user->botInfo.get()) {
      ...
      if (user->starsPerMessage() > 0 ||
          (user->requiresPremiumToWrite() && !user->session().premium()) ||
          user->isFullLoaded()) {
        refresh();
      } else {
        session().api().requestFullPeer(user);
...
bool HistoryInner::isSelected(not_null<SelectedItems *> toItems,
                              not_null<HistoryItem *> item) const {
  const auto i = toItems->find(item);
  return (i != toItems->cend()) && (i->second == FullSelection);
}

bool HistoryInner::isSelectedGroup(not_null<SelectedItems *> toItems,
...
bool HistoryInner::isSelectedAsGroup(not_null<SelectedItems *> toItems,
                                     not_null<HistoryItem *> item) const {
  if (const auto group = session().data().groups().find(item)) {
...
void HistoryInner::addToSelection(not_null<SelectedItems *> toItems,
                                  not_null<HistoryItem *> item) const {
  const auto i = toItems->find(item);
  if (i == toItems->cend()) {
...
void HistoryInner::removeFromSelection(not_null<SelectedItems *> toItems,
                                       not_null<HistoryItem *> item) const {
  const auto i = toItems->find(item);
  if (i != toItems->cend()) {
    toItems->erase(i);
...
void HistoryInner::changeSelection(not_null<SelectedItems *> toItems,
  ...
  if (action == SelectAction::Invert) {
    action = isSelected(toItems, item) ? SelectAction::Deselect
                                       : SelectAction::Select;
  }
  auto total = int(toItems->size());
  const auto add = (action == SelectAction::Select);
  if (add && goodForSelection(toItems, item, total) &&
...
void HistoryInner::changeSelectionAsGroup(not_null<SelectedItems *> toItems,
  ...
  if (action == SelectAction::Invert) {
    action = isSelectedAsGroup(toItems, item) ? SelectAction::Deselect
                                              : SelectAction::Select;
  }
  auto total = int(toItems->size());
...
void HistoryInner::forwardAsGroup(FullMsgId itemId) {
  if (const auto item = session().data().message(itemId)) {
    Window::ShowForwardMessagesBox(_controller,
...
void HistoryInner::deleteAsGroup(FullMsgId itemId) {
  if (const auto item = session().data().message(itemId)) {
    ...
    if (!group) {
      ...
      _controller->show(Box(CreateModerateMessagesBox, group->items, nullptr,
                            ModerateMessagesBoxOptions{}));
    } else {
      _controller->show(Box<DeleteMessagesBox>(
          &session(), session().data().itemsToIds(group->items)));
    }
  }
}
...
void HistoryInner::reportAsGroup(FullMsgId itemId) {
  if (const auto item = session().data().message(itemId)) {
    const auto group = session().data().groups().find(item);
    const auto ids = group ? (ranges::views::all(group->items) |
...
void HistoryInner::blockSenderItem(FullMsgId itemId) {
  if ([[maybe_unused]] const auto item = session().data().message(itemId)) {
    _controller->show(
        Box(Window::BlockSenderFromRepliesBox, _controller, itemId));
  }
}
...
void HistoryInner::addSelectionRange(not_null<SelectedItems *> toItems,
                                     not_null<History *> history, int fromblock,
                                     int fromitem, int toblock,
                                     int toitem) const {
  if (fromblock >= 0 && fromitem >= 0 && toblock >= 0 && toitem >= 0) {
    for (; fromblock <= toblock; ++fromblock) {
      auto block = history->blocks[fromblock].get();
      for (int cnt = (fromblock < toblock) ? block->messages.size()
                                           : (toitem + 1);
           fromitem < cnt; ++fromitem) {
  ...
  if (_dragSelecting) {
    auto fromblock = (_dragSelFrom != botAboutView)
                     ...
                     : _history->blocks.empty() ? -1
                                                : 0;
    auto fromitem =
        (_dragSelFrom != botAboutView) ? _dragSelFrom->indexInBlock()
        : (_history->blocks.empty() || _history->blocks[0]->messages.empty())
            ? -1
            : 0;
    auto toblock = (_dragSelTo != botAboutView)
                   ...
                   : _history->blocks.empty() ? -1
                                              : 0;
    auto toitem =
        (_dragSelTo != botAboutView) ? _dragSelTo->indexInBlock()
        : (_history->blocks.empty() || _history->blocks[0]->messages.empty())
            ? -1
            : 0;
    if (_migrated) {
      if (_dragSelFrom->history() == _migrated) {
        if (_dragSelTo->history() == _migrated) {
          addSelectionRange(toItems, _migrated, fromblock, fromitem, toblock,
                            toitem);
          toblock = -1;
          toitem = -1;
        } else {
          addSelectionRange(toItems, _migrated, fromblock, fromitem,
...
void HistoryInner::onParentGeometryChanged() {
  ...
  auto needToUpdate =
      (_mouseAction != MouseAction::None || _touchScroll || mouseOver);
  if (needToUpdate) {
...
Fn<HistoryView::ElementDelegate *()>
HistoryInner::elementDelegateFactory(FullMsgId itemId) const {
  const auto weak = base::make_weak(_controller);
...
ClickHandlerContext
HistoryInner::prepareClickHandlerContext(FullMsgId itemId) const {
...
ClickContext HistoryInner::prepareClickContext(Qt::MouseButton button,
