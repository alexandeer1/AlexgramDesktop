...
[[nodiscard]] int DefaultIvZoom() {
  ...
  return (std::abs(exact - snap25) <= std::abs(exact - snap10)) ? snap25
                                                                : snap10;
}
...
[[nodiscard]] WindowPosition Deserialize(const QByteArray &data) {
  ...
  stream >> result.x >> result.y >> result.w >> result.h >> result.moncrc >>
      result.maximized >> result.scale;
  return result;
}
...
[[nodiscard]] QByteArray Serialize(const WindowPosition &position) {
  ...
  result.reserve(size);
  {
    ...
    stream.setVersion(QDataStream::Qt_5_1);
    stream << qint32(position.x) << qint32(position.y) << qint32(position.w)
           << qint32(position.h) << qint32(position.moncrc)
           << qint32(position.maximized) << qint32(position.scale);
  }
  return result;
}
...
[[nodiscard]] std::optional<RecentEmojiDocument>
ParseRecentEmojiDocument(const QString &serialized) {
...
rpl::event_stream<bool> &HidePremiumChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &HideAllChatsTabChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &RemoveArchivedFromListChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &HideHelpChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &HidePhoneNumberChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &DisableLinkPreviewByDefaultChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &ShowMessageIdChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &ShowTimestampSecondsChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
rpl::event_stream<bool> &ShowEditedIconChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
...
rpl::event_stream<bool> &ShowOnlineStatusChanges() {
  static rpl::event_stream<bool> stream;
  return stream;
}
...
QByteArray Settings::serialize() const {
  ...
  if (_recentEmojiPreload.empty()) {
    ...
    for (const auto &[id, rating] : _recentEmoji) {
      auto string = QString();
      if (const auto document = std::get_if<RecentEmojiDocument>(&id.data)) {
  ...
  const auto &recentEmojiPreloadData = _recentEmojiPreload.empty()
                                           ...
                                           : _recentEmojiPreload;
  const auto noWarningExtensions =
      QStringList(begin(_noWarningExtensions), end(_noWarningExtensions))
          .join(' ');

  auto size =
      Serialize::bytearraySize(themesAccentColors) + sizeof(qint32) * 5 +
      Serialize::stringSize(_downloadPath.current()) +
      Serialize::bytearraySize(_downloadPathBookmark) + sizeof(qint32) * 9 +
  ...
  size += sizeof(qint32) * 13 + Serialize::bytearraySize(_videoPipGeometry) +
          sizeof(qint32) +
          ...
          sizeof(qint64) + sizeof(qint32) * 2 +
          Serialize::bytearraySize(windowPosition) + sizeof(qint32);
  ...
  size += sizeof(qint32) * 3 + Serialize::bytearraySize(proxy) +
          sizeof(qint32) * 2 + Serialize::bytearraySize(_photoEditorBrush) +
          sizeof(qint32) * 3 +
          Serialize::stringSize(_customDeviceModel.current()) +
          sizeof(qint32) * 4 + (_accountsOrder.size() * sizeof(quint64)) +
          sizeof(qint32) * 7 + (skipLanguages.size() * sizeof(quint64)) +
          sizeof(qint32) * 2 + sizeof(quint64) + sizeof(qint32) * 3 +
          Serialize::bytearraySize(mediaViewPosition) + sizeof(qint32) +
          sizeof(quint64) + sizeof(qint32) * 2;
  ...
  size += sizeof(qint32) * 2 +
          ...
          Serialize::stringSize(_customFontFamily) + sizeof(qint32) * 3 +
          Serialize::bytearraySize(_tonsiteStorageToken) + sizeof(qint32) * 8 +
          sizeof(ushort) + sizeof(qint32) // _notificationsDisplayChecksum
          + Serialize::bytearraySize(callPanelPosition) + sizeof(qint32) * 4;
  size += sizeof(quint32);
  for (const auto &[key, value] : _prefs) {
    size += Serialize::bytearraySize(key) + Serialize::bytearraySize(value);
  }
  size += sizeof(qint32); // _audioPlaybackSpeed
  size += sizeof(qint32); // _showTranslateButton
  ...
  result.reserve(size);
  {
    QDataStream stream(&result, QIODevice::WriteOnly);
    stream.setVersion(QDataStream::Qt_5_1);
    stream << themesAccentColors << qint32(_adaptiveForWide.current() ? 1 : 0)
           << qint32(_moderateModeEnabled ? 1 : 0)
           << qint32(qRound(_songVolume.current() * 1e6))
           << qint32(qRound(_videoVolume.current() * 1e6))
           << qint32(_askDownloadPath ? 1 : 0) << _downloadPath.current()
           << _downloadPathBookmark << qint32(1) << qint32(_soundNotify ? 1 : 0)
    ...
    stream << qint32(_sendFilesWay.serialize())
    ...
    stream << qint32(skipLanguages.size());
    for (const auto &id : skipLanguages) {
      stream << quint64(id.value);
    }

    stream << qint32(_rememberedDeleteMessageOnlyForYou ? 1 : 0)
    ...
    stream << qint32(_trayIconMonochrome.current() ? 1 : 0)
           << qint32(_ttlVoiceClickTooltipHidden.current() ? 1 : 0)
           << _playbackDeviceId.current() << _captureDeviceId.current()
           << _callPlaybackDeviceId.current() << _callCaptureDeviceId.current()
           << ivPosition << noWarningExtensions << _customFontFamily
           << qint32(std::clamp(
                  qRound(_dialogsNoChatWidthRatio.current() * 1000000), 0,
                  1000000))
           << qint32(_systemUnlockEnabled ? 1 : 0)
           << qint32(!_weatherInCelsius   ? 0
                     : *_weatherInCelsius ? 1
                                          : 2)
           ...
           << SerializeVideoQuality(_videoQuality) << qint32(_ivZoom.current())
           << qint32(_systemDarkModeEnabled.current() ? 1 : 0)
           << qint32(_quickDialogAction) << _notificationsVolume
           << _notificationsDisplayChecksum << callPanelPosition
    ...
    stream << qint32(_showTranslateButton.current() ? 1 : 0);
  }
...
void Settings::addFromSerialized(const QByteArray &serialized) {
  ...
  qint32 nativeNotifications =
      _nativeNotifications ? (*_nativeNotifications ? 1 : 2) : 0;
  ...
  qint32 voicePlaybackSpeed =
      SerializePlaybackSpeed(_voicePlaybackSpeed.current());
  ...
  qint32 rememberedDeleteMessageOnlyForYou =
      _rememberedDeleteMessageOnlyForYou ? 1 : 0;
  ...
  qint32 hideAccountName =
      _windowTitleContent.current().hideAccountName ? 1 : 0;
  qint32 hideTotalUnread =
      _windowTitleContent.current().hideTotalUnread ? 1 : 0;
  ...
  qint32 storiesClickTooltipHidden =
      _storiesClickTooltipHidden.current() ? 1 : 0;
  base::flat_set<QString> recentEmojiSkip;
  qint32 trayIconMonochrome = (_trayIconMonochrome.current() ? 1 : 0);
  qint32 ttlVoiceClickTooltipHidden =
      _ttlVoiceClickTooltipHidden.current() ? 1 : 0;
  ...
  qint32 systemAccentColorEnabled = _systemAccentColorEnabled ? 1 : 0;
  qint32 usePlatformTranslation = _usePlatformTranslation ? 1 : 0;
  qint32 systemTextReplace = _systemTextReplace.current() ? 1 : 0;
  qint32 showTranslateButton = _showTranslateButton.current() ? 1 : 0;

  stream >> themesAccentColors;
  if (!stream.atEnd()) {
    stream >> adaptiveForWide >> moderateModeEnabled >> songVolume >>
        videoVolume >> askDownloadPath >> downloadPath >>
        downloadPathBookmark >> nonDefaultVoicePlaybackSpeed >> soundNotify >>
        desktopNotify >> flashBounceNotify >> notifyView >>
        nativeNotifications >> notificationsCount >> notificationsCorner >>
        autoLock >> legacyCallPlaybackDeviceId >> legacyCallCaptureDeviceId >>
        callOutputVolume >> callInputVolume >> callAudioDuckingEnabled >>
        lastSeenWarningSeen >> soundOverridesCount;
    ...
    stream >> sendFilesWay >> sendSubmitWay >> includeMutedCounter >>
        countUnreadMessages >> legacyExeLaunchWarning >> notifyAboutPinned >>
        loopAnimatedStickers >> largeEmoji >> replaceEmoji >> suggestEmoji >>
        suggestStickersByEmoji >> spellcheckerEnabled >> videoPlaybackSpeed >>
        videoPipGeometry >> dictionariesEnabledCount;
    if (stream.status() == QDataStream::Ok) {
    ...
    stream >> autoDownloadDictionaries >> mainMenuAccountsShown;
  }
  if (!stream.atEnd()) {
    auto dialogsWithChatWidthRatioInt = qint32();
    stream >> tabbedSelectorSectionEnabled >> floatPlayerColumn >>
        floatPlayerCorner >> thirdSectionInfoEnabled >>
        dialogsWithChatWidthRatioInt >> thirdColumnWidth >>
        thirdSectionExtendedBy >> notifyFromAll;
    dialogsWithChatWidthRatio =
        std::clamp(dialogsWithChatWidthRatioInt / 1000000., 0., 1.);
  }
  if (!stream.atEnd()) {
  ...
  if (!stream.atEnd()) {
    stream >> groupCallPushToTalk >> groupCallPushToTalkShortcut >>
  ...
  if (!stream.atEnd()) {
    stream >> playerRepeatMode >> playerOrderMode;
  }
  if (!stream.atEnd()) {
  ...
  if (!stream.atEnd()) {
    ...
    if (stream.status() == QDataStream::Ok) {
      for (auto i = 0; i != skipTranslationLanguagesCount; ++i) {
        quint64 language;
        stream >> language;
        skipTranslationLanguages.push_back({QLocale::Language(language)});
      }
    }
  }
  if (!stream.atEnd()) {
  ...
  if (!stream.atEnd()) {
    stream >> translateChatEnabled >> translateToRaw;
  }
  if (!stream.atEnd()) {
    stream >> hideChatName >> hideAccountName >> hideTotalUnread;
  }
  if (!stream.atEnd()) {
  ...
  if (!stream.atEnd()) {
    stream >> playbackDeviceId >> captureDeviceId;
  }
  if (!stream.atEnd()) {
    stream >> callPlaybackDeviceId >> callCaptureDeviceId;
  } else {
  ...
  if (!stream.atEnd()) {
    ...
    stream >> dialogsNoChatWidthRatioInt;
    dialogsNoChatWidthRatio =
        std::clamp(dialogsNoChatWidthRatioInt / 1000000., 0., 1.);
  }
  if (!stream.atEnd()) {
  ...
  case NotifyView::ShowPreview:
    _notifyView = uncheckedNotifyView;
    break;
  }
  ...
  const auto uncheckedNotificationsCorner =
      static_cast<ScreenCorner>(notificationsCorner);
  switch (uncheckedNotificationsCorner) {
  case ScreenCorner::TopLeft:
  case ScreenCorner::TopRight:
  case ScreenCorner::BottomRight:
  case ScreenCorner::BottomLeft:
    _notificationsCorner = uncheckedNotificationsCorner;
    break;
  }
  ...
  _sendFilesWay =
      Ui::SendFilesWay::FromSerialized(sendFilesWay).value_or(_sendFilesWay);
  auto uncheckedSendSubmitWay =
      static_cast<Ui::InputSubmitSettings>(sendSubmitWay);
  switch (uncheckedSendSubmitWay) {
  case Ui::InputSubmitSettings::Enter:
  case Ui::InputSubmitSettings::CtrlEnter:
    _sendSubmitWay = uncheckedSendSubmitWay;
    break;
  }
  ...
  case Window::Column::Third:
    _floatPlayerColumn = uncheckedColumn;
    break;
  }
  ...
  case RectPart::BottomRight:
    _floatPlayerCorner = uncheckedCorner;
    break;
  }
  ...
  case WorkMode::WindowOnly:
    _workMode = uncheckedWorkMode;
    break;
  }
  _hiddenGroupCallTooltips = [&] {
    using Tooltip = Calls::Group::StickedTooltip;
    return Tooltip(0) |
           ((hiddenGroupCallTooltips & int(Tooltip::Camera)) ? Tooltip::Camera
                                                             : Tooltip(0)) |
  ...
  case CloseBehavior::Quit:
    _closeBehavior = uncheckedCloseBehavior;
    break;
  }
  _customDeviceModel = customDeviceModel;
  _accountsOrder = accountsOrder;
  const auto uncheckedPlayerRepeatMode =
      static_cast<Media::RepeatMode>(playerRepeatMode);
  switch (uncheckedPlayerRepeatMode) {
  case Media::RepeatMode::None:
  case Media::RepeatMode::One:
  case Media::RepeatMode::All:
    _playerRepeatMode = uncheckedPlayerRepeatMode;
    break;
  }
  const auto uncheckedPlayerOrderMode =
      static_cast<Media::OrderMode>(playerOrderMode);
  switch (uncheckedPlayerOrderMode) {
  case Media::OrderMode::Default:
  case Media::OrderMode::Reverse:
  case Media::OrderMode::Shuffle:
    _playerOrderMode = uncheckedPlayerOrderMode;
    break;
  }
  ...
  _hardwareAcceleratedVideo = (hardwareAcceleratedVideo == 1);
  {
    ...
    const auto uncheckedChatQuickAction = static_cast<Quick>(chatQuickAction);
    switch (uncheckedChatQuickAction) {
    case Quick::None:
    case Quick::Reply:
    case Quick::React:
      _chatQuickAction = uncheckedChatQuickAction;
      break;
    }
  }
  ...
  _macRoundIconDigest =
      macRoundIconDigest ? macRoundIconDigest : std::optional<uint64>();
...
void Settings::updateDialogsWidthRatio(float64 ratio, bool nochat) {
  const auto changeWithChat = !nochat || (dialogsWithChatWidthRatio() > 0) ||
                              _dialogsWidthSetToZeroWithoutChat;
  const auto changedWithChat =
      changeWithChat && (dialogsWithChatWidthRatio() != ratio);

  const auto changeNoChat = nochat || (dialogsWithChatWidthRatio() != ratio);
  const auto changedNoChat =
      changeNoChat && (dialogsNoChatWidthRatio() != ratio);
...
void Settings::setThirdColumnWidth(int width) { _thirdColumnWidth = width; }
...
rpl::producer<QString> Settings::deviceModelChanges() const {
  return customDeviceModelChanges() | rpl::map([=] { return deviceModel(); });
}

rpl::producer<QString> Settings::deviceModelValue() const {
  return customDeviceModelValue() | rpl::map([=] { return deviceModel(); });
}

int Settings::thirdColumnWidth() const { return _thirdColumnWidth.current(); }
...
void Settings::resolveRecentEmoji() const {
  const auto haveAlready = [&](RecentEmojiId id) {
    return ranges::contains(_recentEmoji, id,
...
void Settings::hideRecentEmoji(RecentEmojiId id) {
  ...
  _recentEmoji.erase(ranges::remove(_recentEmoji, id, &RecentEmoji::id),
                     end(_recentEmoji));
  if (const auto emoji = std::get_if<EmojiPtr>(&id.data)) {
...
bool Settings::ThirdColumnByDefault() { return Platform::IsMacStoreBuild(); }

float64 Settings::DefaultDialogsWidthRatio() {
  return ThirdColumnByDefault() ? kDefaultBigDialogsWidthRatio
...
qint32 Settings::SerializePlaybackSpeed(PlaybackSpeed speed) {
  ...
  const auto value =
      int(base::SafeRound(std::clamp(speed.value, kSpeedMin, kSpeedMax) * 100));
...
bool Settings::skipToastsInFocus() const { return _skipToastsInFocus; }

void Settings::setSkipToastsInFocus(bool value) { _skipToastsInFocus = value; }
...
[[nodiscard]] std::vector<LanguageId>
NonEmptySkipList(std::vector<LanguageId> list) {
...
rpl::producer<LanguageId> Settings::translateToValue() const {
  return _translateToRaw.value() | rpl::map([=](int raw) {
           return raw ? LanguageId{QLocale::Language(raw)}
                      : DefaultSkipLanguages().front();
         }) |
         rpl::distinct_until_changed();
}

void Settings::setSkipTranslationLanguages(std::vector<LanguageId> languages) {
...
int Settings::ivZoom() const { return ResolveIvZoom(_ivZoom.current()); }
...
Media::VideoQuality Settings::videoQuality() const { return _videoQuality; }
